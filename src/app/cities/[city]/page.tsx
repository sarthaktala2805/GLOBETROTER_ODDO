...
export default function CityDetailPage() {
  ...
  useEffect(() => {
    if (cityId) {
      setSaved(savedStore.read().some((item) => item.id === `city:${cityId}`));
    }
  }, [cityId]);

  const nearbyAttractions = useMemo(() => {
    return cityId ? getNearbyAttractions(cityId) : [];
  }, [cityId]);

  const verifiedStays = useMemo(() => {
    if (!cityId) return [];
    const directStays = getAccommodations(cityId);
    if (directStays.length > 0) return directStays;
    return hotels.filter((h) => h.cityId === cityId).map((h) => ({
      id: h.id,
      destinationId: cityId,
      name: h.name,
      type: (h.category.includes("Heritage") ? "Heritage Haveli" : h.category.includes("Resort") ? "Resort" : "Hotel") as any,
  ...
  const connectivity = useMemo(() => {
    return cityId ? getTransportConnectivity(cityId) : undefined;
  }, [cityId]);

  // Compute multi-modal comparison from selected origin hub
    ...
    return (
      <main className="main-content">
        <div style={{ textAlign: "center", padding: "80px 20px" }}>
          <h2 style={{ fontSize: "28px", fontWeight: 700, marginBottom: "12px" }}>Destination Not Found</h2>
  ...
  return (
    <main className="main-content">
      ...
      <div className="detail-grid">
        {/* Left Column: Details, Transport & Attractions */}
        <div>
          {/* Quick Facts Card */}
          ...
          {/* Destination Narrative & Official Verification */}
          <section style={{ marginBottom: "40px" }}>
            ...
            {/* Official Source Badges & Timings */}
            <div
              style={{
                display: "grid",
              ...
              {destMeta?.officialSources && destMeta.officialSources.length > 0 && (
                <div>
                  ...
                  <div style={{ marginTop: "4px" }}>
                    {destMeta.officialSources.map((src) => (
                      <a
          ...
          {/* MULTI-MODAL TRANSPORT COMPARISON ENGINE */}
          <section style={{ marginBottom: "50px" }}>
            ...
            {transitComparison && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "14px" }}>
                {transitComparison.options.map((opt) => (
                  <div
                    key={opt.mode}
                    style={{
                      background: "var(--surface)",
            ...
            {/* Local Station & Airport Connectivity Notes */}
            {connectivity && (
              <div
                style={{
          ...
          {/* Top Attractions / Notable Experiences */}
          <section style={{ marginBottom: "50px" }}>
          ...
          {/* NEARBY EXCURSIONS & ATTRACTIONS (WITH TRAVEL TIMES) */}
                  ...
                  <div key={stay.id} className="city-card">
                    ...
                    <div className="city-card-body" style={{ padding: "16px" }}>
                      ...
                      <div style={{ display: "flex", gap: "4px", flexWrap: "wrap", marginBottom: "12px" }}>
                        {stay.amenities.slice(0, 3).map((am) => (
                          <span
                            key={am}
                            style={{
                              fontSize: "10px",
        ...
        {/* Right Sidebar: Plan Card & Practical Trip Budget */}
        <aside>
          <div
            style={{
              background: "var(--surface)",
          ...
          >
            ...
            {/* Estimated Budget Per Day Breakdown */}
